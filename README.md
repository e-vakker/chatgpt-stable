...
Terminal mode hides the ChatGPT sidebar and chat header by default, removes per-message action bars and non-send composer controls, flattens bubbles/cards/shadows, widens the transcript to a bounded 1120 px terminal column, and starts the Activity Rail collapsed. A tiny `chats` control (or Option-L) toggles the original ChatGPT sidebar without reloading. Terminal also tightens local rendering budgets: long conversations retain 12 recent mounted turns normally, 8 under elevated pressure and 6 under severe pressure; completed agent activity retains 8 ordinary cards and active heavy generation retains 4. Errors and generated artifacts stay visible.
